...
def create_app(test_config=None):
    ...
    def add_question():
        try:
            data = request.json
            question = Question(data.get('question'), data.get('answer'),
    ...
    def quizzes():
        try:
            ...
            category_count = len(
                Question.query.all()) if quiz_category == 0 else len(
                    Question.query.filter_by(category=quiz_category).all())
            if len(pre_question_ids) < category_count:
                new_question = Question.get_new_question(
                    pre_question_ids, quiz_category)
                return jsonify({
                    'success': True,
